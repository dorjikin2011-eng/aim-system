// backend/src/routes/maturity.ts
...
// Extend Express Request type to include user (since your middleware adds it)
...
router.get('/frameworks/indicator/:indicatorId', requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to get maturity framework' 
    });
  }
});
...
router.put('/frameworks/indicator/:indicatorId', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await MaturityFrameworkModel.upsert(
...
router.get('/subsystems/:indicatorId', requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to get subsystems' 
    });
  }
});
...
router.put('/subsystems/:indicatorId', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await MaturityFrameworkModel.updateSubsystems(
      req.params.indicatorId,
      req.body,
      req.user.id
    );
...
router.get('/templates', requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to get framework templates' 
    });
  }
});
