...
export const getDirectorDashboard = async (req: Request, res: Response) => {
  try {
    ...
    // 1. Executive Summary Metrics (SAME as commission)
    const summaryMetrics = await new Promise<any>((resolve, reject) => {
      db.get(
        `SELECT 
    ...
    // 2. Score Distribution Data (SAME as commission)
    const scoreDistribution = await new Promise<any[]>((resolve, reject) => {
      db.all(
        `SELECT 
          CASE 
    ...
    // 3. Sector-wise Performance (SAME as commission)
    const sectorPerformance = await new Promise<any[]>((resolve, reject) => {
      db.all(
        `SELECT 
          ag.sector,
    ...
    // 4. Top and Bottom Agencies (SAME as commission)
    const topAgencies = await new Promise<any[]>((resolve, reject) => {
      db.all(
        `SELECT 
    ...
    const bottomAgencies = await new Promise<any[]>((resolve, reject) => {
      db.all(
        `SELECT 
          ag.name as agency_name,
          ag.sector,
    ...
    // 5. Key Performance Indicators (SAME as commission)
    ...
    // 6. Recent Activity (Last 7 days) - SAME as commission
    ...
    // 7. Workflow Status Overview (SAME as commission)
    const workflowStatus = await new Promise<any[]>((resolve, reject) => {
      db.all(
        `SELECT 
          status,
          COUNT(*) as count
    ...
    res.json({
      summary: {
        totalAgencies: summaryMetrics.total_agencies || 0,
        highIntegrity: summaryMetrics.high_integrity || 0,
        mediumIntegrity: summaryMetrics.medium_integrity || 0,
        lowIntegrity: summaryMetrics.low_integrity || 0,
        avgNationalScore: summaryMetrics.avg_national_score ? parseFloat(summaryMetrics.avg_national_score.toFixed(1)) : 0,
        pendingSubmissions: summaryMetrics.pending_submissions || 0
      },
      ...
      kpis: {
        iccsImplementationRate: kpis.iccs_implementation_rate ? parseFloat(kpis.iccs_implementation_rate.toFixed(1)) : 0,
        trainingCompletionRate: kpis.training_completion_rate ? parseFloat(kpis.training_completion_rate.toFixed(1)) : 0,
        adComplianceRate: kpis.ad_compliance_rate ? parseFloat(kpis.ad_compliance_rate.toFixed(1)) : 0,
        atrTimelinessRate: kpis.atr_timeliness_rate ? parseFloat(kpis.atr_timeliness_rate.toFixed(1)) : 0
      },
...
// Helper functions (SAME as commission)
