...
export const getUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 
        `SELECT 
          id, email, username, full_name as name, role, agency_id,
...
export const createUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existing = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT id FROM users WHERE email = ?', [email], (err, row) => {
    ...
    // Insert ALL columns matching your table schema
    await new Promise<void>((resolve, reject) => {
      await runAsync(db, 
        `INSERT INTO users (
          id, username, email, full_name, password_hash, role, agency_id, 
    ...
    // Send email if requested using shared service
    if (shouldSendEmail) {
      const welcomeHtml = `
      ...
      await transporter.sendMail({
        from: process.env.EMAIL_FROM || `"ACC AIMS" <${process.env.EMAIL_USER}>`,
        to: email,
    ...
    if (err.message?.includes('Failed to send email')) {
      return res.status(500).json({ error: 'User created but email notification failed. Contact system administrator.' });
    }
...
export const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const current = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT email, full_name as name, role, agency_id, phone, department FROM users WHERE id = ?', [id], (err, row) => {
        err ? reject(err) : resolve(row);
      });
    });
...
export const deleteUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT email, full_name as name, role FROM users WHERE id = ?', [id], (err, row) => {
        err ? reject(err) : resolve(row);
      });
    });
    ...
    await new Promise<void>((resolve, reject) => {
      await runAsync(db, 'DELETE FROM users WHERE id = ?', [id], (err) => {
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT email, full_name as name FROM users WHERE id = ?', [id], (err, row) => {
        err ? reject(err) : resolve(row);
      });
    });
