...
export const createAgencyWithHOA = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      await runAsync(db, 'BEGIN TRANSACTION', (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
    ...
    try {
      ...
      if (existingAgency) {
        await new Promise<void>((resolve, reject) => {
          await runAsync(db, 'ROLLBACK', (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
...
export const createAgency = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existing = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT id FROM agencies WHERE LOWER(name) = LOWER(?)', [name.trim()], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
...
export const updateAgency = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const current = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT * FROM agencies WHERE id = ?', [id], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
...
export const deleteAgency = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Get name for audit
    const agency = await new Promise<any>((resolve, reject) => {
      await getAsync(db, 'SELECT name, sector FROM agencies WHERE id = ?', [id], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
    ...
    await new Promise<void>((resolve, reject) => {
      await runAsync(db, 'DELETE FROM agencies WHERE id = ?', [id], (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
