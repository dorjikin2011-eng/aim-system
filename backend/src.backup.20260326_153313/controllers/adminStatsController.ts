...
export const getAdminStats = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const agencies = await new Promise<AgencyCount>((resolve, reject) => {
      await getAsync(db, 'SELECT COUNT(*) as count FROM agencies', (err: Error | null, row: any) => {
        if (err) reject(err);
    ...
    const activeAssessments = await new Promise<AssessmentCount>((resolve, reject) => {
      await getAsync(db, `SELECT COUNT(*) as count 
        FROM assessments 
